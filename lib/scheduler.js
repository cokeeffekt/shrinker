...
const { start, getStatus } = require('./queue');
...
async function runSchedule(entry) {
  ...
  if (status.active) {
    console.log(`[scheduler] Skipping "${entry.directoryName}" — conversion already in progress`);
    return;
  }
  ...
  try {
    ...
    console.log(`[scheduler] Auto-converting ${eligible.length} files in "${entry.directoryName}"`);
    start(eligible, preset).catch(err => {
