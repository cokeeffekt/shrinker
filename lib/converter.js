...
function buildFFmpegArgs(inputPath, outputPath, preset, useNvenc, sourceHeight) {
  const args = useNvenc
  ...
  if (sourceHeight && sourceHeight > preset.height) {
    const scaleFilter = useNvenc
      ? `scale_cuda='min(${preset.width},iw)':'min(${preset.height},ih)':force_original_aspect_ratio=decrease:force_divisible_by=2`
      : `scale='min(${preset.width},iw)':'min(${preset.height},ih)':force_original_aspect_ratio=decrease:force_divisible_by=2`;
...
function convert(inputPath, preset, onProgress, sourceHeight) {
  return new Promise(async (resolve, reject) => {
    const useNvenc = await detectNvenc();
    const tempPath = getTempPath(inputPath);
    const finalPath = getFinalPath(inputPath);
    const args = buildFFmpegArgs(inputPath, tempPath, preset, useNvenc, sourceHeight);
