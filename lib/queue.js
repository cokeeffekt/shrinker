...
function recordHistory(inputPath, file, preset, startTime, outputPath, status) {
  let outputSize = 0;
  if (outputPath) {
...
function convertOne(file, preset) {
  ...
  return convert(inputPath, preset, (prog) => {
    ...
    state.jobs[inputPath].progress = { percent, speed, fps, eta };
  }, file.height || 0)
    .then((outputPath) => {
    ...
    .catch((err) => {
      ...
      recordHistory(inputPath, file, preset, startTime, null, status);
    });
}
...
module.exports = { start, getStatus, cancelActive };
